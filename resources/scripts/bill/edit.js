...
$.templates({
	...
	selectedCinemaListTpl: '<li class="tickets-box-item item-selected item-cinema">'
							+ '<a href="javascript:;">'
							  + '<label title="{{:cinemaName}} - {{:cinemaId}}" data-cinemaid="{{:cinemaId}}">{{:cinemaName}}</label><i onclick="removeSelectedThis(this);"></i>'
							+ '</a>'
						 + '</li>'
});
...
function openCinemaSelectWin(el){
	onCinemaRadioClick($(el).siblings(".cinemaType").children("input[type='radio']")[0]);
