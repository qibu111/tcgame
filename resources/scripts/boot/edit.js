...
var appBootEditFormValidator = $(document.appBootEditForm).validate(validateOptions);
...
function editAppBoot(){
	var valid = appBootEditFormValidator.form();
	if(valid){
		$.ajax({
			async : true,
			cache : false,
			type : 'post',
			dataType : 'json',
			data : $(document.appBootEditForm).serialize(),
			url : document.appBootEditForm.action,
