...
var showAddFormValidator = $(document.showAddForm).validate(validateOptions);

/**
 * 保存演出信息
 * @param action
 */
function addShowInfo(el){
	el = $(el);
	if(el.attr("data-submited")){
		alert("数据正在提交,请勿重复提交表单!");
		return;
	}
	var valid = showAddFormValidator.form();
	if(valid){
		...
		$.ajax({
			...
			type : 'post',
			dataType : 'json',
			data : $(document.showAddForm).serialize(),
			url : document.showAddForm.action,
