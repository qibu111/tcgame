...
( function( $ ) {
    var ranges = {
        ...
        remove = function( elem ) {
            ...
            //ranges[position] -= distance;//固定信息在窗口中央
    ...
    $.floatingMessage = function( message, options ) {
        var id = "jqueryFloatingMessage" + new Date().getTime() + "" + parseInt( Math.random() * 10000, 10 ),
            elem = $( '<div id="'+id+'" class="ui-corner-all ui-floating-message"></div>' ),
            css = {},
            height = 0,
            padding = 0,
            open;
        ...
        css[options.verticalAlign] = ($(window).height() - options.height)/2 - ranges[options.position];
        css[options.align] = ($(window).width() - options.width)/2-align;
        ...
        //ranges[options.position] += ( height + options.margin + ( padding * 2 ) );//固定信息在窗口中央
