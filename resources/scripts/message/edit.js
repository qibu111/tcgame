...
var messageEditFormValidator = $(document.messageEditForm).validate(validateOptions);

function editPushMessage(){
	var valid = messageEditFormValidator.form();
	if(valid){
		...
		if($('input:radio[name=isPush]:checked').val()==2){
			if($('#pushExpiryTime').val().trim()==''){
				messageEditFormValidator.showErrors({pushExpiryTime: '请选择消息推送有效期!'});
				return;
			}
		}
		if($('#messageType').val()==1){
			if(linkType==''){
				messageEditFormValidator.showErrors({linkType: '请选择消息链接页面!'});
				return;
			}
		}
		if($('#messageType').val()!=1){
			if($('#contentSummary').val().trim()==''){
				messageEditFormValidator.showErrors({contentSummary: '请输入内容摘要!'});
				return;
			}
		}
		if(($('#messageType').val()!=1)||($('#messageType').val()==1&&linkType == '0')){
			if($('#content').val().trim()==''){
				messageEditFormValidator.showErrors({content: '请输入消息内容!'});
				return;
			}
		}
		...
		if($('input:radio[name=sendType]:checked').val()==2){
			if($('input:radio[name=sendToP]:checked').val()==1) {
				if ($('#sendUsers').val() == '') {
					messageEditFormValidator.showErrors({sendUsers: '请输入要发送的会员账号名单!'});
					return;
				}
			}else{
				if ($('#sendUsersFile').val() == ''&&!isFile) {
					messageEditFormValidator.showErrors({sendUsersFile: '请上传账号名单文件!'});
					return;
				}

				if ($('#sendUsersFile').val().indexOf(".txt")==-1&&$('#sendUsersFile').val().indexOf(".TXT")==-1) {
					messageEditFormValidator.showErrors({sendUsersFile: '请上传TXT格式账号名单文件!'});
					return;
				}
			}
		}
		if($('#messageType').val()==1){
			if($('#linkType').val()!=0){
				if($('input:radio[name=isPush]:checked').val()==1){
					messageEditFormValidator.showErrors({pushExpiryTime: '是否推送消息必须选择是!'});
					return;
				}
			}
		}
		if(linkType == '2' || linkType == '4' || linkType == '10'){
			...
			$.ajax({
				...
				dataType : 'json',
				data : {
					linkType: document.messageEditForm.linkType.value,
					linkParam: document.messageEditForm.linkParam.value
				},
				url : CONTEXT_PATH + '/message/validate/exists',
				success : function(response) {
					if (response) {
						if(response.success) {
							document.messageEditForm.submit();
						}else{
							messageEditFormValidator.showErrors({linkParam: paramName + '不存在!'});
						}
					} else {
						alert('请求失败!');
					}
...
function onLinkTypeSelected(el){
	...
	messageEditFormValidator.clearError($("#linkParam")[0]);
...
function addImage(imgurl, obj) {
	if(imgurl.indexOf("imageView2/1/w/200")==-1){
...
if(messagePic!=''){
	addImage(messagePic,$('#uploadListPic1'));
}
...
onMessageTypeSelected($('#messageType'));
if($('#messageType').val()==1){
onLinkTypeSelected($("#linkType"));
}
if($("#isPushY").is(':checked')){
	$('#pushExpiryTimeSpan').show();
}else{
	$('#pushExpiryTimeSpan').hide();
}
